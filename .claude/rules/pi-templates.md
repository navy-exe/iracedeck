...
Per-action PI templates and static icons live alongside each action inside `@iracedeck/actions`. Shared partials, the Rollup compile plugin, web components, and the vendored `sdpi-components.js` live in `@iracedeck/pi-components` (the PI framework). Consumer plugins only own their compiled HTML output.
...
└── com.iracedeck.sd.{name}.sdPlugin/
    ...
    └── imgs/actions/<name>/   # Per-action icons copied from @iracedeck/actions (gitignored)
        ├── icon.svg
        └── key.svg
```

Template `require('./data/...')` resolves relative to the shared data directory (`packages/actions/src/actions/data/`), regardless of how deeply the template is nested. The EJS compile plugin rewrites the base path so templates don't need to know their own depth.
...
Import the EJS compile plugin, partials directory, and browser-assets directory from `@iracedeck/pi-components/build`. Compute the templates root locally from `@iracedeck/actions` (each action's folder becomes a template source, so the templates root is `packages/actions/src/actions/`). Add a copy step to emit per-action static icons into the plugin's `imgs/actions/<name>/` (those files are gitignored in the plugin).
...
The plugin's `package.json` must declare both `"@iracedeck/pi-components": "workspace:*"` and `"@iracedeck/actions": "workspace:*"` so pnpm topologically builds them before the plugin.
...
`packages/actions/src/actions/data/docs-urls.json` maps PI template names (without `.ejs`) to their documentation page URLs:
...
- Templates in `packages/actions/src/actions/<name>/<name>.ejs` compile to each plugin's `com.iracedeck.sd.{name}.sdPlugin/ui/<name>.html` (output paths are flat — nesting in the source tree doesn't affect the output filename)
