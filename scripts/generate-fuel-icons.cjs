...
function drawFuelPump(png, cx, cy, scale, color) {
    ...
    // Nozzle curve down
    fillRect(png, Math.round(nozzleStartX + 6 * scale), nozzleStartY, Math.round(4 * scale), Math.round(12 * scale), r, g, b);

    // Nozzle tip
    fillRect(png, Math.round(nozzleStartX + 4 * scale), Math.round(nozzleStartY + 10 * scale), Math.round(8 * scale), Math.round(4 * scale), r, g, b);
    fillRect(png, Math.round(nozzleStartX + 5 * scale), Math.round(nozzleStartY + 14 * scale), Math.round(6 * scale), Math.round(6 * scale), r, g, b);
